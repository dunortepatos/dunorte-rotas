...
function isLogged() {
  return sessionStorage.getItem("dunorte_logged") === "1";
}

function setLogged(v) {
  if (v) sessionStorage.setItem("dunorte_logged", "1");
  else sessionStorage.removeItem("dunorte_logged");
}

function showDashboard() {
  refs.loginScreen.classList.add("hidden");
  refs.dashboardScreen.classList.remove("hidden");
  renderAll();
}

function showLogin() {
  refs.dashboardScreen.classList.add("hidden");
...
refs.loginForm.addEventListener("submit", (e) => {
  ...
  const user = refs.loginUser.value.trim();
  const pass = refs.loginPass.value.trim();
...
function clearRepeats() {
  refs.listaContatos.innerHTML = "";
  refs.listaItens.innerHTML = "";
}
...
function updateReceber() {
  const total = parseMoney(refs.valorTotal.value);
  const entrada = parseMoney(refs.valorEntrada.value);
  const receber = Math.max(total - entrada, 0);
  ...
  if (receber <= 0) {
    refs.formaReceber.value = "";
    refs.bancoOuObsPagamento.value = "";
    refs.parcelamento.value = "";
...
function updatePayFields() {
  ...
  if (needsBank) {
    label.textContent = "Banco / detalhe";
    refs.bancoOuObsPagamento.placeholder = "Ex.: Sicredi, Nubank, Caixa...";
  } else if (needsDetail) {
    label.textContent = "Detalhe";
    refs.bancoOuObsPagamento.placeholder = "Observação opcional";
  }
}
...
refs.fotosPedido.addEventListener("change", async (e) => {
  ...
  for (const file of files) {
    ...
    fotosAtuais.push(data);
  }
  renderPhotoPreview();
  refs.fotosPedido.value = "";
});
...
function renderPhotoPreview() {
  ...
  fotosAtuais.forEach((src, index) => {
    ...
    div.innerHTML = `<img src="${src}" alt="Foto do pedido"><button type="button" data-index="${index}">×</button>`;
...
function collectForm() {
  const contatos = [...refs.listaContatos.querySelectorAll(".contato-input")]
    .map(i => i.value.trim())
    .filter(Boolean);

  const itens = [...refs.listaItens.querySelectorAll(".item-card")]
    .map(card => ({
...
function mapsLink(pedido) {
  return `https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(`${pedido.endereco}, ${pedido.cidade}`)}`;
}

function paymentText(p) {
  const arr = [p.formaReceber];
  if (p.parcelamento) arr.push(p.parcelamento);
  if (p.bancoOuObsPagamento) arr.push(p.bancoOuObsPagamento);
...
function filteredPedidos() {
  ...
  return getPedidos().filter(p => {
    const text = [
      p.nomeCliente, p.numeroPedido, p.cidade, p.bairro, p.endereco, p.loja,
      ...(p.contatos || []),
      ...(p.itens || []).map(i => `${i.quantidade} ${i.nome} ${i.descricao}`)
    ].join(" ").toLowerCase();

    const okTerm = !termo || text.includes(termo);
    const okStatus = !status || p.status === status;
    const okLoja = !loja || p.loja === loja;
    return okTerm && okStatus && okLoja;
  });
}
...
function renderStats(pedidos) {
  ...
  const emRota = pedidos.filter(p => p.status === "EM ROTA DE ENTREGA").length;
  const entregue = pedidos.filter(p => p.status === "ENTREGUE").length;

  refs.statsBar.innerHTML = `
    <div class="stat-card"><div>Total</div><strong>${total}</strong></div>
    <div class="stat-card"><div>Aguardando</div><strong>${aguardando}</strong></div>
    <div class="stat-card"><div>Em rota</div><strong>${emRota}</strong></div>
    <div class="stat-card"><div>Entregues</div><strong>${entregue}</strong></div>
  `;
}
...
function renderPedidos() {
  ...
  refs.listaPedidos.className = "orders-list";
  refs.listaPedidos.innerHTML = pedidos.map(p => `
    <article class="order-card" data-id="${p.id}">
      <div class="order-top">
        <div>
          <h4>${escapeHtml(p.nomeCliente)}</h4>
          <div class="subtext">Pedido ${escapeHtml(p.numeroPedido)} • ${escapeHtml(p.loja)} • Atualizado em ${new Date(p.atualizadoEm).toLocaleString("pt-BR")}</div>
        </div>
        <div class="badge-wrap">
          <span class="badge ${badgeClass(p.status)}">${escapeHtml(p.status)}</span>
          <span class="badge badge-loja">${escapeHtml(p.loja)}</span>
        </div>
      </div>
      ...
      <div class="order-grid">
        <div class="box">
          ...
          <p><strong>Bairro:</strong> ${escapeHtml(p.bairro || "-")}</p>
          <p><strong>Endereço:</strong> ${escapeHtml(p.endereco || "-")}</p>
          <p><strong>Maps:</strong> <a href="${mapsLink(p)}" target="_blank" rel="noopener">Abrir localização</a></p>
          ${p.contatos?.length ? `<p><strong>Contatos:</strong> ${escapeHtml(p.contatos.join(" • "))}</p>` : ""}
        </div>

        <div class="box">
          <h5>Financeiro</h5>
          <p><strong>Total:</strong> ${formatMoney(p.valorTotal)}</p>
          <p><strong>Entrada:</strong> ${formatMoney(p.valorEntrada)}</p>
          ${p.valorReceber > 0 ? `<p><strong>A receber:</strong> ${formatMoney(p.valorReceber)}</p>` : ""}
          ${p.formaReceber ? `<p><strong>Pagamento:</strong> ${escapeHtml(paymentText(p))}</p>` : ""}
          ${p.observacoes ? `<p><strong>Obs.:</strong> ${escapeHtml(p.observacoes)}</p>` : ""}
        </div>
      </div>
      ...
      <div class="box" style="margin-bottom:12px;">
        ...
        ${p.fotos?.length ? `
          <div class="order-photo-cover">
            ${p.fotos.map(src => `<img src="${src}" alt="Foto do pedido">`).join("")}
          </div>
        ` : ""}
      </div>
      ...
      <div class="order-actions">
        ...
        <button class="btn-mini" data-action="status">Mover status</button>
        <button class="btn-mini" data-action="whatsapp">WhatsApp</button>
        <button class="btn-mini danger" data-action="excluir">Excluir</button>

        <div class="sort-box">
          <button class="btn-mini" data-action="subir">↑ Subir</button>
          <button class="btn-mini" data-action="descer">↓ Descer</button>
        </div>
      </div>
    </article>
  `).join("");

  [...refs.listaPedidos.querySelectorAll(".order-card")].forEach(card => {
    card.addEventListener("click", (e) => {
      const action = e.target?.dataset?.action;
      if (!action) return;
      handleCardAction(card.dataset.id, action);
    });
  });
}
...
function handleCardAction(id, action) {
  ...
  if (action === "status") {
    pedidos[i].status = nextStatus(pedidos[i].status);
    pedidos[i].atualizadoEm = new Date().toISOString();
    savePedidos(pedidos);
    renderAll();
    return;
  }
  ...
  if (action === "subir") {
    if (i === 0) return;
    [pedidos[i-1], pedidos[i]] = [pedidos[i], pedidos[i-1]];
    savePedidos(pedidos);
    renderAll();
    return;
  }
  ...
  if (action === "whatsapp") {
    openWhatsApp(orderWhatsAppText(pedidos[i]));
  }
}

function orderWhatsAppText(p) {
  const lines = [
    `*PEDIDO ${p.numeroPedido} - DUNORTE*`,
...
function routePedidos() {
  return filteredPedidos().filter(p => p.status === "EM ROTA DE ENTREGA");
}

function routeItemSummary(items) {
  return "🌳 " + items.map(i => `${i.quantidade} ${pluralizeItem(i.nome, i.quantidade)}`).join(" + ");
}
...
refs.btnRotaWhatsapp.addEventListener("click", () => {
  const pedidos = routePedidos();
  if (!pedidos.length) {
    alert('Não há pedidos com status "EM ROTA DE ENTREGA".');
    return;
  }
  openWhatsApp(routeWhatsAppText());
});

refs.btnImprimirRota.addEventListener("click", () => {
  if (!routePedidos().length) {
    alert('Não há pedidos com status "EM ROTA DE ENTREGA".');
    return;
  }
  refs.filtroStatus.value = "EM ROTA DE ENTREGA";
  renderAll();
  window.print();
});
...
refs.btnExportar.addEventListener("click", () => {
  ...
  a.href = url;
  a.download = "dunorte-pedidos-pro.json";
  a.click();
